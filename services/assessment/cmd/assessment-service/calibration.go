...
func BuildCalibrationPrompt(categories []string) string {
	var parts []string
	parts = append(parts, UniversalAnchors)

	var catParts []string
	for _, cat := range categories {
		if anchor, ok := CategoryAnchors[cat]; ok {
			catParts = append(catParts, anchor)
		}
	}
	if len(catParts) > 0 {
	...
	parts = append(parts, FewShotExamples)
