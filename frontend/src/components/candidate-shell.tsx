...
export function CandidateShell({ children }: CandidateShellProps) {
  ...
  const getInitials = () => {
    ...
    return user?.email ? user.email.slice(0, 2).toUpperCase() : "AR";
  };
  ...
  return (
    <div className="candidate-shell-light flex items-stretch min-h-screen">
      ...
      <aside className="candidate-sidebar w-[248px] flex-none flex flex-col py-6 px-4 sticky top-0 h-screen self-start z-10 border-r border-white/5">
        {/* Sidebar Header */}
        <Link href="/candidate/dashboard" className="flex items-center gap-3 px-2 py-1">
          ...
          <div className="leading-tight">
            ...
            <div className="text-[9.5px] font-semibold tracking-widest text-[#7f8595]">
              {t("workspaceLabel") ? t("workspaceLabel").toUpperCase() : "КАБИНЕТ КАНДИДАТА"}
            </div>
          </div>
        </Link>
        ...
        <nav className="flex flex-col gap-1">
          {menuItems.map((item) => {
            const isActive = pathname === item.href || (item.href === "/candidate/reports" && pathname.startsWith("/candidate/reports/"));
            return (
              <Link
                key={item.href}
                href={item.href}
        ...
        <Link
          ...
          className="mt-6 candidate-btn-primary py-2.5 text-center text-xs font-bold rounded-xl transition-all shadow-[0_4px_12px_rgba(47,91,234,0.3)] hover:shadow-[0_6px_16px_rgba(47,91,234,0.4)]"
        >
          Новое интервью
        </Link>
        ...
        <div className="flex flex-col gap-4 border-t border-white/10 pt-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="leading-tight min-w-0 flex-1">
              ...
              <div className="text-[11px] text-[#7f8595]">{t("steps.startInterviewDesc") ? "Кандидат" : "Candidate"}</div>
            </div>
            <button
              type="button"
              onClick={logout}
