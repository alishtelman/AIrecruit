      ...
      <div className="ai-section mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        {loading ? (
          ...
          <section className="ai-panel mt-6 overflow-hidden rounded-[1.75rem]">
            <div className="grid grid-cols-[1.2fr_1.2fr_0.9fr_0.7fr_0.9fr_1fr] gap-4 border-b border-slate-800 px-6 py-4 text-xs uppercase tracking-[0.22em] text-slate-500">
              <div>{t("table.candidate")}</div>
              <div>{t("table.role")}</div>
              <div>{t("table.recommendation")}</div>
              <div>{t("table.score")}</div>
              <div>{t("table.created")}</div>
              <div>{t("table.reportId")}</div>
            </div>
            <div className="divide-y divide-slate-800">
              {items.map((item) => (
                <div key={item.id} className="grid grid-cols-[1.2fr_1.2fr_0.9fr_0.7fr_0.9fr_1fr] gap-4 px-6 py-4 text-sm">
                  <div>
                  ...
                  <div className="truncate text-slate-500">{item.id}</div>
                </div>
              ))}
            </div>
          </section>
